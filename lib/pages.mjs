...
export function orgHome({ site, i18n, lang, editions }) {
  ...
  const heroImage = site.shareImage;
...
<section class="hero org-hero">
  ...
  <figure class="org-hero-visual">
    <img src="${escUrl(heroImage)}" alt="" width="1600" height="1066" decoding="async" />
    ${next ? `<figcaption><span>${esc(o.nextEditionLabel)}</span><b>${esc(next.year)}</b></figcaption>` : ""}
  </figure>
</section>
...
function editionCard(edition, lang, site, i18n) {
  ...
  return `<li class="edition ${upcoming ? "is-upcoming" : "is-past"}">
    <a href="${esc(href)}">
      ${cover
        ? `<span class="edition-cover"><img src="${escUrl(cover)}" alt="" loading="${upcoming ? "eager" : "lazy"}"${upcoming ? ' fetchpriority="high"' : ""} decoding="${upcoming ? "sync" : "async"}" /></span>`
        : `<span class="edition-cover edition-cover-placeholder" aria-hidden="true"><i>IndabaX Burundi</i><b>${esc(edition.year)}</b></span>`}
...
function editionNavigation(edition, editions, site, i18n, lang) {
  const chronological = [...(editions || [])].sort((a, b) => a.year - b.year);
  const current = chronological.findIndex((item) => item.year === edition.year);
  if (current < 0) return "";

  const previous = chronological[current - 1];
  const next = chronological[current + 1];
  if (!previous && !next) return "";

  const link = (item, direction, label, arrow) => item ? `
    <a class="edition-nav-link is-${direction}" href="${esc(editionHref(item, lang, site))}">
      <span>${direction === "previous" ? `${arrow} ` : ""}${esc(label)}${direction === "next" ? ` ${arrow}` : ""}</span>
      <b>${esc(item.year)}</b>
    </a>` : "";

  return `<nav class="edition-nav" aria-label="${esc(i18n.event.editionNavigation)}">
    ${link(previous, "previous", i18n.event.previousEdition, "←")}
    ${link(next, "next", i18n.event.nextEdition, "→")}
  </nav>`;
}

export function eventPage({ site, i18n, lang, edition, editions }) {
  const e = i18n.event;
...
${editionNavigation(edition, editions, site, i18n, lang)}
